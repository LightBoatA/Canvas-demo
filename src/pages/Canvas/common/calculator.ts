 ...
 * @returns
 */
export const getCtrlPoints = (rect: IRect): ICtrlPoint[] => {
 ...
 * @param shape
 */
export const getIntersectedControlPoint = (pointX: number, pointY: number, rect: IRect | null) => {
  if (rect) {
 ...
 * @param cursorX 
 * @param cursorY 
 * @param resizeStartInfo 
 * @returns 被选图形id和新数据的对应map
 */
export const calcResizedShape = (cursorX: number, cursorY: number, resizeStartInfo: IResizeStartInfo) => {
  const { oldBox, oldSelectedShapes, direction } = resizeStartInfo;
  const { x, y, width, height } = oldBox!;

  const { left, top, bottom, right } = getRectBounds(oldBox!);
  // 计算选框的新位置、新宽高
  let newX = x,
    newY = y,
    newWidth = width,
  ...
  const newShapes = oldSelectedShapes.map(shape => {
    const newShapeX = newX + (shape.x - x) * scaleX; // (shape.x - x) * scaleX 形状相对于选框的新位置
    const newShapeY = newY + (shape.y - y) * scaleY;
    const newShapeWidth = shape.width * scaleX;
    const newShapeHeight = shape.height * scaleY;

    // 根据新的形状数据，计算连接点数据
    const newConnectionPoints = shape.connectionPoints.map(point => {
      return getConnectionPointVal(newShapeX, newShapeY, newShapeWidth, newShapeHeight, point.direction);
    });
...
export const getIntersectedConnectionPoint = (shapes: IShape[], x: number, y: number) => {
  for (let i = 0; i < shapes.length; i++) {
    ...
    for (let j = 0; j < points.length; j++) {
      if (Math.abs(x - points[j].x) < 3 && Math.abs(y - points[j].y) < 3) {
